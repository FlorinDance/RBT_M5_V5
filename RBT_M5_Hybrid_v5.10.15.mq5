//+------------------------------------------------------------------+
//| RBT_M5_Hybrid_v5.10.15.mq5                                      |
//| Normal RBT + live recovery and trailing management.            |
//+------------------------------------------------------------------+
#property strict
#property version   "5.115"
#property description "RBT M5 v5.10.15 - live recovery and trailing; optional entry risk diagnostics"
...
int OnInit()
{
   if(!RecoveryInit()) { Print("Recovery inputs or CSV initialization failed"); return INIT_FAILED; }
   string inputReason = "";
   ...
   PrintFormat("RBT M5 HYBRID v5.10.15 ready | mode=NORMAL_PLUS_RISK_SHADOW | risk_shadow=%d threshold=%.3f reduced_mult=%.3f | manager_dataset=%d/%ds | XGB BUY=%.3f SELL=%.3f GAP=%.3f | motif=%s | base lot=%.2f fixedSL=%.2f | policy_lab=%d prudent_A=%.1f/%.1f prudent_O=%.1f/%.1f permissive_A=%.1f/%.1f permissive_O=%.1f/%.1f",
