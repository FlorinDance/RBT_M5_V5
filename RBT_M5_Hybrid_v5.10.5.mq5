//+------------------------------------------------------------------+
//| RBT_M5_Hybrid_v5.10.5.mq5                                       |
//| XGBoost entry direction + frozen hourly Motif risk assessment.   |
//+------------------------------------------------------------------+
#property strict
#property version   "5.105"
#property description "RBT M5 v5.10.5 - hybrid + scalper profit protector research"
...
int OnInit()
{
   ...
   PrintFormat("RBT M5 HYBRID v5.10.5 ready | mode=%s | XGB BUY=%.3f SELL=%.3f GAP=%.3f | motif=%s | base lot=%.2f fixedSL=%.2f | protect=%d arm=%.2f floor=%.2f",
...
void OnTick()
{
   ...
   if(!allowTrade)
   {
      HybridCSVLogEntry(decision,pSell,pHold,pBuy,chosen,gap,motifState,
         g_runtimeLots,lotMultiplier,0.0,g_runtimeTakeProfitMoney,
         tpMultiplier,0.0,(InpUseDynamicSL ? -1.0 : InpStopLossPips),
