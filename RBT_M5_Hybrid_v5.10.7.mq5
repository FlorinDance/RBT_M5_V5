//+------------------------------------------------------------------+
//| RBT_M5_Hybrid_v5.10.7.mq5                                       |
//| XGBoost entry + Motif quality risk and adaptive profit protect.  |
//+------------------------------------------------------------------+
#property strict
#property version   "5.107"
#property description "RBT M5 v5.10.7 - adaptive Motif profit protector"
...
int OnInit()
{
   ...
   PrintFormat("RBT M5 HYBRID v5.10.7 ready | mode=%s | XGB BUY=%.3f SELL=%.3f GAP=%.3f | motif=%s | base lot=%.2f fixedSL=%.2f | protect=%d adaptive=%d agree=%.2f/%.2f oppose=%.2f/%.2f fallback=%.2f/%.2f",
      (InpScalperMode ? "XGB_SCALPER" : "NORMAL_HYBRID"),
      InpMinBuyProb, InpMinSellProb, InpMinDecisionGap,
      (InpHybridMotifEnable && (!InpScalperMode || InpHybridMotifRiskInScalper) ?
       "QUALITY_RISK_ACTIVE" : "OFF"),
      g_runtimeLots,(InpScalperMode ? InpScalperStopLossPips : InpStopLossPips),
      (int)(InpScalperMode && InpScalperProfitProtectEnable),
      (int)InpScalperMotifAdaptiveProtectEnable,
      InpScalperMotifAgreeArmMoney,InpScalperMotifAgreeFloorMoney,
      InpScalperMotifOpposeArmMoney,InpScalperMotifOpposeFloorMoney,
      InpScalperProfitProtectArmMoney,InpScalperProfitProtectFloorMoney);
...
void OnTick()
{
   ...
   const double baseTPMoney = (InpScalperMode ? InpScalperTakeProfitMoney :
   ...
   const bool opened = OpenTradeFromDecision(decision, pSell, pHold, pBuy, regPred,
      ...
      (InpScalperMode ? effectiveTPMoney : 0.0));
   HybridCSVLogEntry(decision,pSell,pHold,pBuy,chosen,gap,motifState,
